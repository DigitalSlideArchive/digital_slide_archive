...
# /opt/venv/bin/python3 /opt/digital_slide_archive/devops/dsa/provision.py --worker-pre -v --yaml /opt/provision.yaml
/opt/venv/bin/python3 /opt/provision.py --worker-pre -v --yaml /opt/provision.yaml
echo ==== Provisioning === &&
# PATH="/opt/venv/bin:/.pyenv/bin:/.pyenv/shims:$PATH" \
# /opt/venv/bin/python3 /opt/digital_slide_archive/devops/dsa/provision.py --worker-main -v --yaml /opt/provision.yaml
...
DOCKER_CLIENT_TIMEOUT=86400 TMPDIR=${TMPDIR:-/tmp} GW_DIRECT_PATHS=true /opt/venv/bin/python3 -m girder_worker --concurrency=${DSA_WORKER_CONCURRENCY:-2} -Ofair --prefetch-multiplier=1
# "
